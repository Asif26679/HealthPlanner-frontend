...
export default function Dashboard() {
  ...
  const handleExportReport = () => {
    ...
    // Title
    doc.setFontSize(18);
    doc.text("Daily Diet Report", 105, 20, { align: "center" });
    ...
    // Totals
    doc.setFontSize(14);
    doc.text("Total Nutrition", 20, 50);
    doc.setFontSize(12);
    doc.text(`Calories: ${diet.totalCalories} kcal`, 20, 60);
    doc.text(`Protein: ${diet.totalProtein} g`, 20, 70);
    doc.text(`Carbs: ${diet.totalCarbs} g`, 20, 80);
    doc.text(`Fats: ${diet.totalFats} g`, 20, 90);
  
    // Meals
    let startY = 105;
    diet.meals.forEach((meal) => {
      doc.setFontSize(14);
      doc.text(`${meal.name}`, 20, startY);
      startY += 6;
  
      doc.setFontSize(12);
      meal.items.forEach((item) => {
        doc.text(`- ${item.name}: ${item.calories} kcal`, 25, startY);
        startY += 6;
      });
  
      startY += 4; // space between meals
    });
  
    // Footer
    doc.setFontSize(10);
    doc.text("Generated by HealthPlanner", 105, 290, { align: "center" });
