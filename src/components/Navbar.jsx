...
export default function Navbar() {
  ...
  const navItems = ["Home", "Plans", "Workouts", "About"];
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          {user ? (
            <div className="relative group">
              ...
              {/* Hover Dropdown */}
              <div className="absolute right-0 mt-2 w-60 bg-neutral-900 text-white rounded-xl shadow-lg 
                              opacity-0 translate-y-2 invisible group-hover:opacity-100 group-hover:translate-y-0 
                ...
                <div className="flex flex-col gap-2 p-4">
                  ...
                  <Button
                    ...
                    className="mt-2 flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg transition"
                  >
            ...
            >
              <nav className="flex flex-col gap-6 mt-4">
                ...
                <div className="flex justify-end mb-4">
                  <SheetTrigger asChild>
                    <Button variant="ghost" size="icon" className="text-gray-200 hover:text-green-400">
                      <Menu className="h-6 w-6 rotate-45" /> {/* rotated to act as close */}
                    </Button>
                  </SheetTrigger>
                </div>
                ...
                {navItems.map((item, idx) => (
                  ...
                  >
                    {item === "Plans" && <LayoutDashboard className="w-5 h-5 text-green-400" />}
                    {item === "Workouts" && <Dumbbell className="w-5 h-5 text-blue-400" />}
                    {item === "Home" && <User className="w-5 h-5 text-yellow-400" />}
                    {item === "About" && <Droplets className="w-5 h-5 text-blue-300" />}
                    {item}
                  </Link>
                ))}
                ...
                {user && (
                  <div className="mt-4 border-t border-neutral-700 pt-4 space-y-3">
                    ...
                    <Link
                      to="/dashboard"
                      className="flex items-center gap-2 px-4 py-3 rounded-xl bg-green-600 hover:bg-green-500 text-white font-medium transition"
                    >
                    ...
                    <Button
                      onClick={handleLogout}
                      className="w-full mt-2 bg-red-500 hover:bg-red-600 text-white px-4 py-3 rounded-xl transition font-semibold"
                    >
